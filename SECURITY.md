...
- **Define a Rules-of-Engagement file** (`.crucible-roe.yaml`; see
  `.crucible-roe.example.yaml`). When present, CRUCIBLE **refuses** any live target or
...
- The offensive paths (`--recon` / `--full-stack` / `--extract` / `--discover`)
  ...
  `CRUCIBLE_AUTHORIZED=1` for automation (`--ci` does **not** bypass them).
