# CRUCIBLE (AI Red Team CLI) — Roadmap v6
...
| G16 | **Slice E — safety-posture completion** (the rest of the P0 safety table). (E1) `roe.py` + `--roe`/`--no-roe`/`--roe-override` — a **Rules-of-Engagement / scope-confinement file** (`.ai-redteam-roe.yaml`, CIDR/IP/host/URL + expiry); when present, every live target and recon scope is confined to the authorized list, enforced at the `engine`/`multiturn` transport (loopback always allowed) **and** up-front in each side-effectful handler (`scan`/`recon`/`extract`/`discover`/`crescendo`/`multi-turn`). (E2) `audit.py` — an **append-only audit trail** (`.ai-redteam-audit.jsonl`): operator, timestamp, action, target **host-only** (never URL/creds), mode, ROE ref, authorized/override — one record per side-effectful run. (E3) `reporter.scrub_pii()` scrubs emails/SSNs/API-key-secrets/phones from saved response + payload bodies under `--anonymize` (the model-stealing engine elicits these; `--anonymize` must not archive them verbatim). `.ai-redteam-roe.example.yaml` documents the format. +13 tests; verified live (out-of-scope target refused + audited). | L | ✅ |
...
| G18 | **Slice H — packaging & adoption.** Single-sourced the version (`main.__version__ = "3.0.0"`) across the banner/description/docstring and added a `--version` action — fixes the 4-way version drift (v2.2/v2.4/v3.0/3.0.0); parser `prog` is now `redai` (matches the console script). **Rewrote README.md**: correct install (`pip install -e ".[all]"` for `--serve`, was the broken `requirements.txt`), leads with the `redai` command, a 60-sec quickstart, the full current 20-mode table (was 13, wrong counts) + a flagship-capabilities section (adaptive/local-first/SLM/recon/full-stack/safety), accurate flag reference and examples. Added **CHANGELOG.md**. +2 tests (version single-source guard). | M | ✅ |
