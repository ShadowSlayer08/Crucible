...
| G14 | **Slice D — finish the full-stack merge** (infra ↔ behaviour, one tool, one report). (D1) `reporter.save_sarif(infra_findings=…)` folds AgentHound findings into the SARIF run (rules + results tagged `layer:infra`, ATLAS/OWASP), so CI code-scanning surfaces both layers. (D2) `--full-stack` now **runs the behavioural sweep** — `fullstack.sweep_targets()` fires CRUCIBLE's red-team at each discovered endpoint via subprocess self-invocation (injectable/testable; `--no-sweep` keeps the plan-only path). (D3) `fullstack.build_unified_report()` merges infra findings/endpoints/paths + behavioural summaries into ONE `fullstack_<ts>.json` (+ `.sarif`) with a combined risk posture. (D4) `fullstack.correlate()` chains the layers (reachable **and** exploitable → escalated CRITICAL, linked to the infra finding id); `render_attack_paths()` emits an ASCII chain + Mermaid graph from the previously-unused path edges; `targets.save_target(extra=…)` persists recon context (service/mode/auth/source) so findings attribute back. +7 tests; verified live end-to-end (recon → sweep → unified JSON+SARIF). | L | ✅ |
| G15 | **Slice F — runtime robustness** (correctness bugs that skewed the ASR metric). (F1) `engine.run_test` now retries **429 + 5xx/529 + ConnectionError** with exponential backoff + jitter, honoring a server `Retry-After` header — a transient failure no longer becomes a permanent false `ERROR` that deflates FAIL counts. (F2) configurable request **timeout** (`engine.set_timeout()` + `config["timeout"]` + `--timeout`) replaces the hardcoded 30s, so slow long-context/reasoning answers aren't turned into ERRORs. (F3) `BudgetTracker.try_reserve()` makes the check-and-count a single critical section (`record_usage()` no longer double-counts), so `--max-calls` is a **hard ceiling** instead of overshooting by up to (concurrency-1) under `--concurrency`. +15 tests. | M | ✅ |
| G16 | **Slice E — safety-posture completion** (the rest of the P0 safety table). (E1) `roe.py` + `--roe`/`--no-roe`/`--roe-override` — a **Rules-of-Engagement / scope-confinement file** (`.crucible-roe.yaml`, CIDR/IP/host/URL + expiry); when present, every live target and recon scope is confined to the authorized list, enforced at the `engine`/`multiturn` transport (loopback always allowed) **and** up-front in each side-effectful handler (`scan`/`recon`/`extract`/`discover`/`crescendo`/`multi-turn`). (E2) `audit.py` — an **append-only audit trail** (`.crucible-audit.jsonl`): operator, timestamp, action, target **host-only** (never URL/creds), mode, ROE ref, authorized/override — one record per side-effectful run. (E3) `reporter.scrub_pii()` scrubs emails/SSNs/API-key-secrets/phones from saved response + payload bodies under `--anonymize` (the model-stealing engine elicits these; `--anonymize` must not archive them verbatim). `.crucible-roe.example.yaml` documents the format. +13 tests; verified live (out-of-scope target refused + audited). | L | ✅ |
| G20 | **run() dispatch-registry refactor** (the XL architecture cleanup). The ~1600-line `run()` god-function (71 `sys.exit`, no unit-testable path) is split into three cohesive, return-based functions: a 46-line `run()` = fixed setup prologue (color/seed/_apply_local/rate/timeout/ROE/evolve) → `code = _dispatch(args)` → `run_main_pipeline(args)`; **`_dispatch(args) -> int|None`** holds the ordered early-exit subcommand chain (returns an exit code, or None to fall through); **`run_main_pipeline(args) -> int`** holds the behavioural pipeline. All `sys.exit(n)` became `return n`; `main()` is now the single exit point (`sys.exit(run(args))`). Ordering (incl. the load-bearing config-load between handler groups) preserved verbatim. Guarded by a **golden exit-code test** (`tests/test_run_dispatch.py`, argv→code incl. exit-4 ROE) written first and kept green across all 4 increments; full suite green throughout. | XL | ✅ |
| G19 | **Slice I — anti-forgetting training recipe** (the one **critical**-impact audit item: the root cause of the flagship KEEP-base A/B, Δ-33.3%). `slm/train.py` reworked so a fine-tune stops regressing the base: (1) LoRA targets default to **`all-linear`** (incl. MLP gate/up/down_proj) via `resolve_lora_targets()` — was attention-only; presets `attention`/`mlp`/`all-linear` + custom comma-list. (2) optional **DoRA / rsLoRA** (`--dora`/`--rslora`), passed to `LoraConfig` only when the installed PEFT accepts them (version-safe introspection). (3) **general-instruction replay mixing** (`mix_replay` + a built-in `default_replay_set()` of benign tasks + `--replay-file` for Alpaca/Dolly) so replay forms ~`--replay-ratio` (default 0.3) of the training set — the actual anti-forgetting mechanism; on by default (`--no-replay` restores the old behaviour). (4) small-attack-set warning. Pure recipe functions unit-tested (+11); the fine-tune itself stays GPU-operator (`slm.pipeline` now trains with the recipe by default). | L | ✅ |
| G18 | **Slice H — packaging & adoption.** Single-sourced the version (`main.__version__ = "3.0.0"`) across the banner/description/docstring and added a `--version` action — fixes the 4-way version drift (v2.2/v2.4/v3.0/3.0.0); parser `prog` is now `crucible` (matches the console script). **Rewrote README.md**: correct install (`pip install -e ".[all]"` for `--serve`, was the broken `requirements.txt`), leads with the `crucible` command, a 60-sec quickstart, the full current 20-mode table (was 13, wrong counts) + a flagship-capabilities section (adaptive/local-first/SLM/recon/full-stack/safety), accurate flag reference and examples. Added **CHANGELOG.md**. +2 tests (version single-source guard). | M | ✅ |
| G17 | **Slice E4 — supply-chain hardening.** `.github/dependabot.yml` (weekly `pip` + `github-actions` update PRs) and a new `.github/workflows/ci.yml`: runs the **full pytest suite on every push/PR** across Python 3.10/3.11/3.12 (previously nothing proved the suite green on a commit), plus a supply-chain job — **pip-audit** dependency-vulnerability scan + a **CycloneDX SBOM** artifact. `SECURITY.md` updated to document the ROE file, per-run authorization, the audit trail, rate limiting, and `--anonymize` PII scrubbing. | M | ✅ |
